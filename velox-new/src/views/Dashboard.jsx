...
function RegimeBar({ regime, loading }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:20, padding:'10px 20px', background:'var(--bg2)', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
      {loading ? (
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <Loader size={13} color="var(--text3)" style={{ animation:'spin 1s linear infinite' }} />
          <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--text3)' }}>FETCHING LIVE REGIME...</span>
        </div>
      ) : (
        <>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
...
function SectorHeatmap({ sectors, loading }) {
  const fallback = ['Technology','Industrials','Health Care','Financials','Cons. Disc.','Materials','Energy','Utilities','Real Estate','Cons. Staples'].map(s=>({sector:s,rs:0}));
  const display = loading || !sectors?.length ? fallback : sectors;
  return (
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <span style={{ fontFamily:'var(--mono)', fontSize:10, color:'var(--text2)', letterSpacing:'0.1em' }}>SECTOR RELATIVE STRENGTH {loading?'· LOADING...':'· LIVE'}</span>
        <span style={{ fontFamily:'var(--mono)', fontSize:9, color:'var(--text3)' }}>3-MONTH vs SPY</span>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:6 }}>
        {display.slice(0,10).map((s,i) => {
          const rs = s.rs||0;
          const intensity = Math.abs(rs)/40;
          const bg = loading?'var(--bg3)':rs>15?`rgba(29,158,117,${0.15+intensity*0.5})`:rs>0?`rgba(239,159,39,${0.1+intensity*0.3})`:`rgba(226,75,74,${0.1+intensity*0.4})`;
          const col = loading?'var(--text3)':rs>15?'var(--teal)':rs>0?'var(--amber)':'var(--red)';
          return (
            <div key={s.sector} style={{ background:bg, border:`1px solid ${col}33`, borderRadius:'var(--radius)', padding:'8px 10px', position:'relative' }}>
              {!loading&&i<3&&<div style={{ position:'absolute', top:4, right:6, width:5, height:5, borderRadius:'50%', background:'var(--teal)' }} />}
              <div style={{ fontSize:10, color:'var(--text2)', marginBottom:3, lineHeight:1.2 }}>{s.sector}</div>
              <div style={{ fontFamily:'var(--mono)', fontSize:13, fontWeight:600, color:col }}>{loading?'···':`${rs>0?'+':''}${rs}%`}</div>
            </div>
          );
        })}
      </div>
      {!loading&&<div style={{ marginTop:10, display:'flex', alignItems:'center', gap:6 }}><div style={{ width:5, height:5, borderRadius:'50%', background:'var(--teal)' }} /><span style={{ fontFamily:'var(--mono)', fontSize:9, color:'var(--text3)' }}>TOP SECTOR — SCREENER PRIORITY</span></div>}
    </div>
  );
}
...
function RankedOpportunities({ onSelectStock }) {
  ...
  return (
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', overflow:'hidden' }}>
      ...
      {top.map((s,i) => (
        <div key={s.ticker} onClick={()=>onSelectStock(s.ticker)} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 16px', borderBottom:i<top.length-1?'1px solid var(--border)':'none', cursor:'pointer' }}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            ...
            <span style={{ fontSize:9, fontFamily:'var(--mono)', padding:'2px 7px', borderRadius:10, background:s.setup==='Breakout'?'var(--amber-bg)':s.setup==='Pullback'?'var(--red-bg)':'var(--bg3)', color:s.setup==='Breakout'?'var(--amber)':s.setup==='Pullback'?'var(--red)':'var(--text2)' }}>{s.setup}</span>
  ...
  return (
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', overflow:'hidden' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr' }}>
        {sorted.map((s,i) => (
          <div key={s.ticker} onClick={()=>onSelectStock(s.ticker)} style={{ padding:'10px 16px', borderRight:i%2===0?'1px solid var(--border)':'none', borderBottom:i<sorted.length-2?'1px solid var(--border)':'none', cursor:'pointer' }}
            onMouseEnter={e=>e.currentTarget.style.background='var(--bg3)'}
            onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontFamily:'var(--mono)', fontWeight:600, fontSize:13 }}>{s.ticker}</span>
              <span style={{ fontFamily:'var(--mono)', fontSize:12, color:s.chg>=0?'var(--teal)':'var(--red)' }}>{s.chg>=0?'+':''}{s.chg}%</span>
            </div>
